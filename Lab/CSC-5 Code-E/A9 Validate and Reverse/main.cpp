 ...
 * File:   main.cpp
 * Author: Jason Handen
 * Created on March 13, 2021, 8:02 PM
...
bool  inRange(const char digits[],unsigned short &unShort){
    char leng=strlen(digits);
    
    if(leng>5){
        return false;
    }else if(leng==5&&digits[0]>54){
        return false;
    }else if(leng==5&&digits[0]==54){
        if(digits[1]>53){
            return false;
    ...
    for(char i=0;i<leng;i++){
        if(digits[i]<48||digits[i]>57){
            return false;
        }
    }
    unShort = (unsigned short)strtoul(digits, NULL, 0);
    return true;
}
bool  reverse(unsigned short unShort,signed short &snShort){
    
    int numb=unShort, temp=0, reverse=0;
    ...
    if(unShort<10){
        ...
        reverse*=1000;
    }else if(unShort<1000){
        reverse*=100;
    }else if(unShort<10000){
        reverse*=10;
    }
